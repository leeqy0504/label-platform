...
import { ChevronLeft, ExternalLink, Download, AlertTriangle, RefreshCw, Box } from 'lucide-react';
...
export default function TrainingDetailPage() {
  ...
  const configEntries: [string, string][] = [
    ...
    ['Weight Decay', String(run.config.weightDecay)],
    ['Warmup Epochs', String(run.config.warmupEpochs)],
  ];
    ...
    <div className="p-6 max-w-screen-xl mx-auto space-y-5">
      ...
      <div>
        ...
        <div className="flex items-start justify-between">
          ...
          <div className="flex items-center gap-2">
            <button onClick={load} className="size-7 flex items-center justify-center text-gray-400 hover:bg-gray-100 rounded">
      ...
      {run.status === 'running' && (
        <div className="bg-white border border-gray-200 rounded-lg p-4">
          ...
          <Progress value={(run.currentEpoch / run.totalEpochs) * 100} className="h-2" />
          <div className="flex items-center gap-4 mt-2">
            <span className="text-xs text-gray-500">GPU 利用率: <strong className="text-gray-800">{run.gpuUtil}%</strong></span>
            <span className="text-xs text-gray-500">显存: <strong className="text-gray-800">{run.gpuMem.toFixed(1)} GB</strong></span>
            ...
            <div className="bg-white border border-gray-200 rounded-lg p-4">
              ...
              <div className="space-y-2">
                {[
                  ...
                  ['mAP50-95', (run.primaryMetric * 0.62).toFixed(3)],
